...
- (GLBDataContentView*)contentView {
    if(_contentView == nil) {
        ...
        CGFloat width = (contentSize.width > FLT_EPSILON) ? contentSize.width : self.frame.size.width;
        CGFloat height = (contentSize.height > FLT_EPSILON) ? contentSize.height : self.frame.size.height;
        _contentView = [[GLBDataContentView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, width, height)];
...
- (NSArray*)visibleItems {
    [_visibleItems sortUsingComparator:^NSComparisonResult(GLBDataItem* item1, GLBDataItem* item2) {
...
- (NSArray*)visibleCells {
    ...
    for(GLBDataItem* item in self.visibleItems) {
...
- (NSArray*)selectedCells {
    NSMutableArray* result = NSMutableArray.array;
    for(GLBDataItem* item in _selectedItems) {
...
- (NSArray*)highlightedCells {
    NSMutableArray* result = NSMutableArray.array;
    for(GLBDataItem* item in _highlightedItems) {
        GLBDataCell* cell = item.cell;
        if(cell != nil) {
            [result addObject:cell];
        }
...
- (NSArray*)editingCells {
    NSMutableArray* result = NSMutableArray.array;
    for(GLBDataItem* item in _editingItems) {
        GLBDataCell* cell = item.cell;
        if(cell != nil) {
            [result addObject:cell];
        }
...
- (GLBDataItem*)movingItem {
    return _movingItem;
}

- (GLBDataCell*)movingCell {
...
- (void)setSearchBar:(GLBSearchBar*)searchBar {
    if(_searchBar != searchBar) {
        ...
        self.searchBarInset = (_showedSearchBar == YES) ? _searchBar.glb_frameHeight : 0.0f;
    }
}
...
- (BOOL)shouldDeselectItem:(GLBDataItem* __unused)item {
...
- (BOOL)shouldUnhighlightItem:(GLBDataItem* __unused)item {
    return YES;
}
...
- (void)unhighlightAllItemsAnimated:(BOOL)animated {
    if(_highlightedItems.count > 0) {
        [_highlightedItems glb_each:^(GLBDataItem* item) {
            if([self shouldUnhighlightItem:item] == YES) {
                [_highlightedItems removeObject:item];
                [item unhighlightedAnimated:animated];
            }
...
- (BOOL)shouldEndedEditItem:(GLBDataItem* __unused)item {
...
- (BOOL)shouldEndedMoveItem:(GLBDataItem* __unused)item {
    ...
    if(_updating == NO) {
        if(duration > FLT_EPSILON) {
            ...
            [GLBTimeout executeBlock:^{
                [self _batchComplete:complete animated:YES];
            } afterDelay:duration];
        } else {
            [self _batchUpdate:^{
                if(update != nil) {
                ...
                [self _batchComplete:complete animated:NO];
            } animated:NO];
        }
    } else {
        [_queueBatch addObject:[[GLBDataBatch alloc] initWithDuration:duration update:update complete:complete]];
    }
}
...
- (GLBDataCell*)_dequeueCellWithItem:(GLBDataItem*)item {
    ...
    GLBDataCell* cell = [queue lastObject];
    if(cell == nil) {
        ...
        if(cell != nil) {
            ...
            [self.contentView.subviews enumerateObjectsUsingBlock:^(UIView* view, NSUInteger index, BOOL* stop) {
                if([view isKindOfClass:GLBDataCell.class] == YES) {
                    GLBDataCell* existCell = (GLBDataCell*)view;
...
- (void)_validateLayout {
    ...
    if(_container != nil) {
        layoutRect = [_container _validateLayoutForAvailableFrame:GLBRectMakeOriginAndSize(CGPointZero, self.glb_frameSize)];
    }
...
- (void)_didScrollDragging:(BOOL)dragging decelerating:(BOOL)decelerating {
    if(self.pagingEnabled == NO) {
        ...
        CGSize contentViewSize = self.contentView.glb_frameSize;
        if(self.bounces == YES) {
        ...
        if(_topRefreshView != nil) {
            ...
            if((_topRefreshIteractionEnabled == YES) && (contentViewSize.height > 0.0f)) {
        ...
        if(_bottomRefreshView != nil) {
            CGFloat progress = 0.0f;
            if((_bottomRefreshIteractionEnabled == YES) && (contentViewSize.height >= frameSize.height)) {
                CGFloat inset = _edgeInset.bottom + _containerInset.bottom;
                CGFloat limit = (contentViewSize.height - frameSize.height) - inset;
        ...
        if(_leftRefreshView != nil) {
            CGFloat progress = 0.0f;
            if((_leftRefreshIteractionEnabled == YES) && (contentViewSize.width >= FLT_EPSILON)) {
        ...
        if(_rightRefreshView != nil) {
            CGFloat progress = 0.0f;
            if((_rightRefreshIteractionEnabled == YES) && (contentViewSize.width >= frameSize.width)) {
                CGFloat inset = _edgeInset.right + _containerInset.right;
                CGFloat limit = (contentViewSize.width - frameSize.width) - inset;
...
- (void)_batchUpdate:(GLBSimpleBlock)update animated:(BOOL)animated {
    ...
    [self validateLayoutIfNeed];
    [self _layoutForVisible];
    [_container _endUpdateAnimated:animated];
    ...
    if(_reloadedAfterItems.count > 0) {
        if([self containsActionForKey:GLBDataViewAnimateReplaceIn] == YES) {
            ...
            for(GLBDataItem* item in _reloadedAfterItems) {
                GLBDataCell* cell = item.cell;
                if(cell != nil) {
                    [UIView performWithoutAnimation:^{
                        cell.glb_ZPosition = -1.0f;
    ...
    if(_insertedItems.count > 0) {
        if([self containsActionForKey:GLBDataViewAnimateInsert] == YES) {
            ...
            for(GLBDataItem* item in _insertedItems) {
                GLBDataCell* cell = item.cell;
                if(cell != nil) {
                    [UIView performWithoutAnimation:^{
                        cell.glb_ZPosition = -1.0f;
...
- (void)_handlerLongPressGestureRecognizer:(UILongPressGestureRecognizer*)gestureRecognizer {
    ...
    switch(gestureRecognizer.state) {
        case UIGestureRecognizerStateBegan: {
            GLBDataItem* item = [self itemForPoint:location];
