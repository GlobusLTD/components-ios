...
+ (UINib*)glb_nibWithClass:(Class)class bundle:(NSBundle*)bundle {
    UINib* nib = [self glb_nibWithName:NSStringFromClass(class) bundle:bundle];
    if((nib == nil) && (class.superclass != nil)) {
        nib = [self glb_nibWithClass:class.superclass bundle:bundle];
    }
    return nib;
}
...
+ (UINib*)glb_cacheNibForClass:(Class)class {
    return [self glb_cacheNibForName:NSStringFromClass(class)];
}
